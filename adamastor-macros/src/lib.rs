...
pub fn prompt(_attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let (input_type, wrapper_struct) = if inputs.is_empty() {
            ...
            .map(|(name, ty)| {
                ...
                quote! {
                    properties.insert(#name_str.to_string(),
                        <#ty as adamastor::GeminiSchema>::gemini_schema());
        ...
        let arg_names: Vec<_> = inputs
            ...
            .collect();
        let arg_types: Vec<_> = inputs.iter().map(|(_, ty)| ty).collect();

        let from_impl = quote! {
            impl From<(#(#arg_types),*)> for #wrapper_name {
                fn from(args: (#(#arg_types),*)) -> Self {
                    let (#(#arg_names),*) = args;
                    Self {
                        #(#arg_names),*
                    }
        ...
        let wrapper = quote! {
            ...
            #from_impl // The generated From trait is added here

            impl adamastor::GeminiSchema for #wrapper_name {
                fn gemini_schema() -> serde_json::Value {
                    let mut properties = serde_json::Map::new();
                    let mut required = vec![];
    ...
    let expanded = quote! {
        ...
        impl adamastor::IntoPrompt for #fn_name {
            type Input = #input_type;
            type Output = #output_type;

            fn into_prompt(self) -> adamastor::Prompt<Self::Input, Self::Output> {
                adamastor::Prompt::new(#template_fn)
            }
        }
    };
...
pub fn tool(_attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        ...
        impl adamastor::IntoTool for #fn_name {
            fn into_tool(self) -> Box<dyn adamastor::GeminiTool> {
                Box::new(adamastor::tool(#fn_name_str, |input: #input_type| async move {
                    #call_expr
                }))
            }
        }
